...
// Fetch today's sales
$todaySalesResult = $conn->query("SELECT SUM(total) as total FROM client WHERE DATE(created_at) = CURDATE()");
$todaySales = $todaySalesResult ? $todaySalesResult->fetch_assoc()['total'] : 0;

// Fetch current month's sales
$currentMonth = date('m');
$monthSalesResult = $conn->query("SELECT SUM(total) as total FROM client WHERE MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = YEAR(CURDATE())");
$monthSales = $monthSalesResult ? $monthSalesResult->fetch_assoc()['total'] : 0;

// Fetch current year's sales
$currentYear = date('Y');
$yearSalesResult = $conn->query("SELECT SUM(total) as total FROM client WHERE YEAR(created_at) = '$currentYear'");
$yearSales = $yearSalesResult ? $yearSalesResult->fetch_assoc()['total'] : 0;

// Sales data for the current year
$salesDataResult = $conn->query("SELECT MONTH(created_at) as month, SUM(total) as total FROM client WHERE YEAR(created_at) = '$currentYear' GROUP BY MONTH(created_at)");
...
$currentMonth = date('m');
$monthlySalesDataResult = $conn->query("SELECT DAY(created_at) as day, SUM(total) as total FROM client WHERE MONTH(created_at) = '$currentMonth' AND YEAR(created_at) = '$currentYear' GROUP BY DAY(created_at)");
